...
class vpn:
    def __init__(self, name, key, peer, ti, remote_enc_domain, source_enc_domain, local_sources, dports, local_server, lports, settings):
    #def __init__(self, settings):
        self.vpn_settings = json.loads(settings)
        self.name = self.vpn_settings["vpn"]["vpn_name"]
        self.key = self.vpn_settings["ike_policy"]["pre_shared_key"]
        self.peer = self.vpn_settings["ike_gateway"]["address"]
        self.ti = self.vpn_settings["vpn_tunnel_definition"]["secure_interface"]
...
def main():
    new_vpn = vpn(sys.argv[1], sys.argv[2], sys.argv[3], sys.argv[4], sys.argv[5], sys.argv[6], sys.argv[7].split(','), sys.argv[8].split(','), sys.argv[9], sys.argv[10].split(','), input_file)
