...
#PHASE-1
    def phase_1(self):
        ike = '' +\
        '\nset security ike proposal %s description %s' % (self.ike_proposal["name"], self.vpn["description"]) +\
...
#PHASE-2
    def phase_2(self):
        ipsec = '' +\
        '\nset security ipsec proposal %s protocol esp' % self.vpn_name +\
        '\nset security ipsec proposal %s authentication-algorithm hmac-sha1-96' % self.vpn_name +\
        '\nset security ipsec proposal %s encryption-algorithm aes-256-cbc' % self.vpn_name +\
        '\nset security ipsec proposal %s lifetime-seconds 3600' % self.vpn_name +\
        '\n' +\
        '\nset security ipsec policy %s_IPSEC_POLICY proposals %s' % (self.vpn_name, self.vpn_name)
...
#IKE GATEWAY
    ...
    def ike_gateway(self):
        ikegateway = '' +\
        '\nset security ike gateway %s ike-policy %s' % (self.vpn_name, self.vpn_name) +\
        '\nset security ike gateway %s address %s' % (self.vpn_name, self.peer) +\
        '\nset security ike gateway %s dead-peer-detection always-send' % self.vpn_name +\
        '\nset security ike gateway %s external-interface reth1.404' % self.vpn_name +\
        '\nset security ike gateway %s general-ikeid' % self.vpn_name
...
#VPN TUNEL DEFINITION
    def vpn(self):
        vpntunnel = '' +\
        '\nset security ipsec vpn %s bind-interface st0.%s' % (self.vpn_name, self.si) +\
        '\nset security ipsec vpn %s ike gateway %s' % (self.vpn_name, self.si) +\
        '\nset security ipsec vpn %s ike ipsec-policy %s' % (self.vpn_name, self.ipsec_policy_name) +\
        '\nset security ipsec vpn %s establish-tunnels immediately' % self.vpn_name
...
#INTERFACE TUNEL DEFINITION
    def tunel_interface(self):
        ti = '' +\
        '\nset interfaces st0 unit %s description \"VPN %s\"' % (self.si, self.vpn_name) +\
        '\nset interfaces st0 unit %s family inet mtu 1436' % self.si +\
        '\nset security zones security-zone DMZ_VPN interfaces st0.%s' % self.si
...
#STATIC ROUTE
    def static_route(self):
        comando = ""
        for ip in self.remote_enc_domains:
